...
public class JsGridReportBase {
	 ...
	 */
	public void exportToExcel(String title, TableData tableData,String agent)
			throws Exception {
		...
		wb = writeSheet(wb,title,styles,tableData);//写入工作表
		...
		for (int i = 1; i <= excel_num; i++) {
			...
			for(int j = 1; j <= sheet_num; j++){
				...
				if(bean.getChildren()!=null && bean.getChildren().length>0)
					td = ExcelUtil.createTableData(sheetLst, ExcelUtil.createTableHeader(bean.getHearders(),bean.getChildren()),bean.getFields());
				else
					td = ExcelUtil.createTableData(sheetLst, ExcelUtil.createTableHeader(bean.getHearders()),bean.getFields());
